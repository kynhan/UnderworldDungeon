...
class SteamStatsAndAchievements : MonoBehaviour {
	...
	private Achievement_t[] m_Achievements = new Achievement_t[] {
		new Achievement_t(Achievement.ACH_BONEARMOR, "Bone Armor", "Unlocked a new armor set"),
		new Achievement_t(Achievement.ACH_STYXPENNY, "Penny of Styx", "Unlocked a new item"),
		new Achievement_t(Achievement.ACH_CLOSECALL, "Close Call", "Kill a boss with 100 health or less remaining"),
		new Achievement_t(Achievement.ACH_TITANSHARD, "Titan Shard", "Unlocked a new item"),
		new Achievement_t(Achievement.ACH_LORD, "Lord of the Underworld", "Complete the game and get every achievement"),
		new Achievement_t(Achievement.ACH_STREAK, "Streak!", "Beat the game 3 times in a row without dying"),
		new Achievement_t(Achievement.ACH_DECKEDOUT, "Decked out", "Fill all gear slots during a run"),
		new Achievement_t(Achievement.ACH_AEGIS, "Aegis", "Unlocked a new item"),
		new Achievement_t(Achievement.ACH_HADESBIDENT, "Hades' Bident", "Unlocked a new item"),
		new Achievement_t(Achievement.ACH_ZEUSBOLT, "Zeus' Bolt", "Unlocked a new item"),
		new Achievement_t(Achievement.ACH_CRYSTALSHELL, "Crystal Shell", "Unlocked a new item"),
		new Achievement_t(Achievement.ACH_OBELISKBLADE, "Obelisk Blade", "Unlocked a new item"),
		new Achievement_t(Achievement.ACH_ORACLE, "Oracle", "Unlocked a new character"),
	};
	...
	private void Update() {
		...
		//Unlock Lord Achievement (every achievement)
		bool allItems = true; bool allAchs = true; bool allCharComplete = true;

		for(int i=0;i<GameControlScript.control.unlockedItems.Length;i++) {
			if(GameControlScript.control.unlockedItems[i] == 0){
				allItems = false;
			}
		}
		for(int i=0;i<GameControlScript.control.achUnlocked.Length;i++) {
			if(GameControlScript.control.achUnlocked[i] == 0){
				if(i!=4){ allAchs = false; }
			}
		}
		for(int i=0;i<GameControlScript.control.characterComplete.Length;i++) {
			if(GameControlScript.control.characterComplete[i] == 0){
				allCharComplete = false;
			}
		}
		if(allItems && allAchs && allCharComplete){GameControlScript.control.achUnlocked[4] = 1;}
		...
		foreach (Achievement_t achievement in m_Achievements) {
			...
			switch (achievement.m_eAchievementID) {
				case Achievement.ACH_BONEARMOR:
					if (GameControlScript.control.achUnlocked[0] == 1) {
						UnlockAchievement(achievement);
					}
				break;
				case Achievement.ACH_STYXPENNY:
					if (GameControlScript.control.achUnlocked[1] == 1) {
						UnlockAchievement(achievement);
					}
				break;
				case Achievement.ACH_CLOSECALL:
					if (GameControlScript.control.achUnlocked[2] == 1) {
						UnlockAchievement(achievement);
					}
				break;
				case Achievement.ACH_TITANSHARD:
					if (GameControlScript.control.achUnlocked[3] == 1) {
						UnlockAchievement(achievement);
					}
				break;
				case Achievement.ACH_LORD:
					if (GameControlScript.control.achUnlocked[4] == 1) {
						UnlockAchievement(achievement);
					}
				break;
				case Achievement.ACH_STREAK:
					if (GameControlScript.control.achUnlocked[5] == 1) {
						UnlockAchievement(achievement);
					}
				break;
				case Achievement.ACH_DECKEDOUT:
					if (GameControlScript.control.achUnlocked[6] == 1) {
						UnlockAchievement(achievement);
					}
				break;
				case Achievement.ACH_AEGIS:
					if (GameControlScript.control.achUnlocked[7] == 1) {
						UnlockAchievement(achievement);
					}
				break;
				case Achievement.ACH_HADESBIDENT:
					if (GameControlScript.control.achUnlocked[8] == 1) {
						UnlockAchievement(achievement);
					}
				break;
				case Achievement.ACH_ZEUSBOLT:
				if (GameControlScript.control.achUnlocked[9] == 1) {
					UnlockAchievement(achievement);
				}
				break;
				case Achievement.ACH_CRYSTALSHELL:
				if (GameControlScript.control.achUnlocked[10] == 1) {
					UnlockAchievement(achievement);
				}
				break;
				case Achievement.ACH_OBELISKBLADE:
				if (GameControlScript.control.achUnlocked[11] == 1) {
					UnlockAchievement(achievement);
				}
				break;
				case Achievement.ACH_ORACLE:
